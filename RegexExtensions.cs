...
namespace CodeMechanic.RegularExpressions
{
    /// <summary>
    /// TODO List:
    /// 1. Try these suggestions: https://medium.com/@mohsen_rajabi/how-to-write-a-regex-very-fast-in-c-best-practice-875d386c0485
    /// 2. Create a Benchmark project and verify the benchmarks expressed here: https://devblogs.microsoft.com/dotnet/regular-expression-improvements-in-dotnet-7/#source-generation
    /// </summary>
    public static class RegexExtensions
    {
        ...
        {
            var dict = replacements.ToDictionary(tuple => tuple.Item1,
                tuple => tuple.Item2);
        ...
        /// <returns></returns>
        public static List<T> AutoExtract<T>(
            this string text,
            string delimiter = "",
        ...
        )
        {
            ...
            foreach (var kvp in dictionary)
            {
                ...
                string named_group_pattern =
                        @$"(?<{propertyname}>{pattern}){delimiter}" /*.Dump("named group")*/
                    ;
            ...
            var batch = text.Extract<T>(full_pattern,
                enforce_exact_match: false, options: options);

            return batch;
        }

        ///
        /// From: https://stackoverflow.com/questions/6005609/replace-only-some-groups-with-regex
        /// Usage:
        /// ```cs
        ...
        )
        {
            return regex.Replace(
                ...
                match =>
                {
                    ...
                    {
                        var currentCaptureEnd = capture.Index + capture.Length -
                                                match.Index;
                        var currentCaptureLength = capture.Index - match.Index -
                                                   previousCaptureEnd;
